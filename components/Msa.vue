...
const setupSectionObservers = () => {
  if (!process.client) return
  const sectionIds = ['career', 'api-public', 'projects', 'articles', 'connect']
...
function downloadCV() {
  ...
  const handleMessage = (event: MessageEvent) => {
    if (event.data && event.data.type === 'cv-downloaded') {
      cleanupWithTimeout()
    }
  }
...
<template>
  <div
    class="bg-[#0a0b0b] min-h-screen text-[#e3e2e2] selection:bg-[#00f0ff] selection:text-[#002022] font-geist antialiased overflow-x-clip relative">

    <!-- ═══ Scroll Progress Bar ═══ -->
    <div class="fixed top-0 left-0 w-full h-[2px] z-[100]">
      <div
        class="h-full bg-gradient-to-r from-[#00f0ff] via-[#7df4ff] to-[#00f0ff] transition-all duration-150 ease-out shadow-[0_0_10px_rgba(0,240,255,0.5)]"
    ...
    <header class="fixed top-0 w-full z-50 transition-all duration-500"
      :class="scrollProgress > 2 ? 'bg-[#0a0b0b]/95 backdrop-blur-2xl border-b border-[#3b494b]/15 shadow-[0_4px_30px_rgba(0,0,0,0.3)]' : 'bg-transparent'">
      <div class="flex justify-between items-center h-16 md:h-20 px-5 md:px-16 max-w-[1400px] mx-auto w-full">
        ...
        <a class="font-hanken text-xl md:text-2xl font-bold tracking-tight text-[#e3e2e2] group relative" href="#">
          ...
          <span
            class="font-geist text-[10px] font-semibold tracking-[0.3em] text-[#00f0ff]/60 ml-2 uppercase hidden md:inline">Portfolio</span>
          <span
            class="absolute -bottom-1 left-0 w-0 h-[2px] bg-[#00f0ff] group-hover:w-full transition-all duration-300"></span>
        </a>
        ...
        <nav class="hidden md:flex items-center gap-1">
          <a v-for="link in navLinks" :key="link.id"
            ...
            :href="link.href">
            {{ link.label }}
            <span v-if="isActiveLink(link.id)"
              class="absolute bottom-0 left-1/2 -translate-x-1/2 w-4 h-[2px] bg-[#00f0ff] rounded-full"></span>
          </a>
        </nav>
        ...
        <div class="flex items-center gap-3">
          <!-- Locale Switcher -->
          <div
            class="hidden sm:flex items-center gap-1 bg-[#1a1b1b]/60 border border-[#3b494b]/20 rounded-full px-1 py-1">
          ...
          <a :href="cvDownloadUrl" @click.prevent="downloadCV"
            class="hidden sm:inline-flex items-center gap-2 bg-[#00f0ff] text-[#002022] hover:bg-[#7df4ff] px-5 py-2.5 rounded-full font-geist text-[10px] font-bold tracking-wider hover:scale-[0.97] transition-all duration-300 uppercase shadow-[0_0_20px_rgba(0,240,255,0.15)] hover:shadow-[0_0_30px_rgba(0,240,255,0.3)]">
            {{ $t('download_cv') }}
          </a>

          <!-- Mobile Menu Toggle -->
          <button @click="toggleMobileMenu"
            class="md:hidden flex flex-col items-center justify-center w-10 h-10 rounded-lg border border-[#3b494b]/30 bg-[#1a1b1b]/40 transition-all duration-300">
            <span class="block w-4 h-[1.5px] bg-[#e3e2e2] transition-all duration-300"
              :class="isMobileMenuOpen ? 'rotate-45 translate-y-[3px]' : ''"></span>
            <span class="block w-4 h-[1.5px] bg-[#e3e2e2] mt-[3px] transition-all duration-300"
              :class="isMobileMenuOpen ? '-rotate-45 -translate-y-[3px]' : ''"></span>
          </button>
        </div>
      </div>
      ...
      <Transition name="slide-down">
        <div v-if="isMobileMenuOpen" class="md:hidden border-t border-[#3b494b]/15 bg-[#0a0b0b]/98 backdrop-blur-2xl">
          <nav class="flex flex-col p-4 gap-1">
            ...
            <a :href="cvDownloadUrl" @click.prevent="downloadCV"
              class="mt-2 text-center bg-[#00f0ff] text-[#002022] px-5 py-3 rounded-xl font-geist text-xs font-bold tracking-wider uppercase">
              {{ $t('download_cv') }}
            </a>
          </nav>
        </div>
    ...
    <main class="relative z-10" :style="panoramicStyle">
      ...
      <ScrollyTellingCanvas :frame-count="183" frame-prefix="/me/ezgif-frame-" :force-desktop="connectedTabsCount > 0"
        :vertical-offset="panoramicOffsetY" v-slot="{ progress, isLoading }">
        <!-- Particle Canvas -->
        <canvas id="particle-canvas" ref="canvasRef"
          class="absolute inset-0 pointer-events-none z-0 transition-opacity duration-1000"
          :class="isLoading ? 'opacity-0' : 'opacity-100'"></canvas>

        <div class="absolute inset-0 hero-gradient-overlay pointer-events-none z-0 transition-opacity duration-1000"
          :class="isLoading ? 'opacity-0' : 'opacity-100'"></div>

        <!-- Glowing Orb -->
        <div
          class="absolute -left-20 top-1/4 w-[450px] h-[450px] bg-[#00f0ff]/8 blur-[150px] rounded-full pointer-events-none z-0 animate-pulse transition-opacity duration-1000"
          :class="isLoading ? 'opacity-0' : 'opacity-100'"></div>
        <div
          class="absolute right-10 bottom-1/4 w-[300px] h-[300px] bg-[#ff0080]/5 blur-[120px] rounded-full pointer-events-none z-0 animate-pulse transition-opacity duration-1000"
          :class="isLoading ? 'opacity-0' : 'opacity-100'"></div>
        ...
        <div v-if="!isLoading" class="absolute inset-0 pointer-events-none flex flex-col justify-center">
          ...
          <div v-show="progress >= -0.1 && progress <= 0.25"
            ...
            <div class="flex flex-col gap-6 max-w-[620px] w-full"
              :class="connectedTabsCount > 0 ? 'text-left' : 'text-center lg:text-left'">
              <div
                class="inline-flex items-center gap-2.5 px-4 py-2 rounded-full border border-[#3b494b]/40 bg-[#1a1b1b]/60 backdrop-blur-md w-fit"
                :class="connectedTabsCount > 0 ? 'mx-0' : 'mx-auto lg:mx-0'">
                <span
                  class="w-2 h-2 rounded-full bg-[#00f0ff] animate-pulse shadow-[0_0_8px_rgba(0,240,255,0.6)]"></span>
                <span class="font-geist text-[10px] text-[#b9cacb] uppercase tracking-[0.2em] font-semibold">{{
                  $t('badge_frontend_engineer') }}</span>
              </div>
              <h1 class="font-hanken font-bold text-[#e3e2e2] leading-[1.05] tracking-tight"
                :class="connectedTabsCount > 0 ? 'text-6xl' : 'text-4xl md:text-5xl lg:text-[4rem]'">
                {{ $t('hero_title') }}
              </h1>
              <p class="font-geist text-[#b9cacb] max-w-xl font-light leading-[1.8]"
                :class="connectedTabsCount > 0 ? 'text-lg mx-0' : 'text-base md:text-lg mx-auto lg:mx-0'">
                {{ $t('hero_subtitle') }}
              </p>
              <div class="flex flex-wrap items-center gap-4 mt-4 pointer-events-auto"
                :class="connectedTabsCount > 0 ? 'justify-start' : 'justify-center lg:justify-start'">
                <button @click="discoverMSA"
                  class="group relative bg-[#00f0ff] text-[#002022] hover:bg-[#7df4ff] px-8 py-4 rounded-full font-geist text-xs font-bold uppercase tracking-wider transition-all duration-300 shadow-[0_0_25px_rgba(0,240,255,0.2)] hover:shadow-[0_0_40px_rgba(0,240,255,0.35)] hover:scale-105 overflow-hidden">
                  <span class="relative z-10 flex items-center gap-2">
                    {{ $t('explore_work') }}
                    <UIcon name="i-heroicons-arrow-down-20-solid" class="w-4 h-4 animate-bounce" />
                  </span>
                  <div
                    class="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-700 skew-x-12">
                  </div>
                </button>
                <a href="#connect"
                  class="group border border-[#3b494b]/60 text-[#e3e2e2] hover:border-[#00f0ff]/50 hover:bg-[#00f0ff]/5 px-8 py-4 rounded-full font-geist text-xs font-bold uppercase tracking-wider transition-all duration-300 hover:scale-105 hover:shadow-[0_0_20px_rgba(0,240,255,0.1)]">
          ...
          <div v-show="progress >= 0.15 && progress <= 0.45"
            ...
            <div class="max-w-[620px] w-full"
              :class="connectedTabsCount > 0 ? 'text-left' : 'text-center lg:text-left'">
              <div
                class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-[#3b494b]/30 bg-[#1a1b1b]/40 w-fit mb-6"
                :class="connectedTabsCount > 0 ? 'mx-0' : 'mx-auto lg:mx-0'">
                <span class="w-1.5 h-1.5 rounded-full bg-[#00f0ff]"></span>
                <span class="font-geist text-[9px] text-[#b9cacb] uppercase tracking-[0.2em] font-semibold">Core
                  Stack</span>
              </div>
              <h2 class="font-hanken font-bold tracking-tight text-[#e3e2e2] mb-8"
                :class="connectedTabsCount > 0 ? 'text-5xl' : 'text-3xl md:text-5xl'">
                Tech Stack
              </h2>
              <div class="grid gap-3 pointer-events-auto w-full"
                :class="connectedTabsCount > 0 ? 'grid-cols-4' : 'grid-cols-3 sm:grid-cols-4'">
                <div
                  class="tech-card flex flex-col items-center gap-3 p-4 rounded-xl border border-[#3b494b]/20 bg-[#1a1b1b]/30 backdrop-blur-md hover:border-[#00f0ff]/50 hover:scale-[1.05] hover:shadow-[0_0_20px_rgba(0,240,255,0.08)] transition-all duration-400 cursor-default group">
                  <UIcon name="logos:react"
                    class="w-10 h-10 group-hover:drop-shadow-[0_0_8px_rgba(97,218,255,0.5)] transition-all duration-300" />
                  <span
                    class="text-[10px] font-geist text-[#b9cacb] tracking-wider font-semibold group-hover:text-[#00f0ff] transition-colors duration-300">React</span>
                </div>
                <div
                  class="tech-card flex flex-col items-center gap-3 p-4 rounded-xl border border-[#3b494b]/20 bg-[#1a1b1b]/30 backdrop-blur-md hover:border-[#00f0ff]/50 hover:scale-[1.05] hover:shadow-[0_0_20px_rgba(0,240,255,0.08)] transition-all duration-400 cursor-default group">
                  <div
                    class="w-10 h-10 bg-white rounded-full flex items-center justify-center group-hover:drop-shadow-[0_0_8px_rgba(255,255,255,0.3)] transition-all duration-300">
                    <UIcon name="logos:nextjs-icon" class="w-6 h-6" />
                  </div>
                  <span
                    class="text-[10px] font-geist text-[#b9cacb] tracking-wider font-semibold group-hover:text-[#00f0ff] transition-colors duration-300">Next.js</span>
                </div>
                <div
                  class="tech-card flex flex-col items-center gap-3 p-4 rounded-xl border border-[#3b494b]/20 bg-[#1a1b1b]/30 backdrop-blur-md hover:border-[#00f0ff]/50 hover:scale-[1.05] hover:shadow-[0_0_20px_rgba(0,240,255,0.08)] transition-all duration-400 cursor-default group">
                  <UIcon name="logos:vue"
                    class="w-10 h-10 group-hover:drop-shadow-[0_0_8px_rgba(66,184,131,0.5)] transition-all duration-300" />
                  <span
                    class="text-[10px] font-geist text-[#b9cacb] tracking-wider font-semibold group-hover:text-[#00f0ff] transition-colors duration-300">Vue</span>
                </div>
                <div
                  class="tech-card flex flex-col items-center gap-3 p-4 rounded-xl border border-[#3b494b]/20 bg-[#1a1b1b]/30 backdrop-blur-md hover:border-[#00f0ff]/50 hover:scale-[1.05] hover:shadow-[0_0_20px_rgba(0,240,255,0.08)] transition-all duration-400 cursor-default group">
                  <UIcon name="logos:nuxt-icon"
                    class="w-10 h-10 group-hover:drop-shadow-[0_0_8px_rgba(0,220,130,0.5)] transition-all duration-300" />
                  <span
                    class="text-[10px] font-geist text-[#b9cacb] tracking-wider font-semibold group-hover:text-[#00f0ff] transition-colors duration-300">Nuxt</span>
                </div>
                <div
                  class="tech-card flex flex-col items-center gap-3 p-4 rounded-xl border border-[#3b494b]/20 bg-[#1a1b1b]/30 backdrop-blur-md hover:border-[#00f0ff]/50 hover:scale-[1.05] hover:shadow-[0_0_20px_rgba(0,240,255,0.08)] transition-all duration-400 cursor-default group">
                  <UIcon name="logos:tailwindcss-icon"
                    class="w-10 h-10 group-hover:drop-shadow-[0_0_8px_rgba(56,189,248,0.5)] transition-all duration-300" />
                  <span
                    class="text-[10px] font-geist text-[#b9cacb] tracking-wider font-semibold group-hover:text-[#00f0ff] transition-colors duration-300">Tailwind</span>
                </div>
                <div
                  class="tech-card flex flex-col items-center gap-3 p-4 rounded-xl border border-[#3b494b]/20 bg-[#1a1b1b]/30 backdrop-blur-md hover:border-[#00f0ff]/50 hover:scale-[1.05] hover:shadow-[0_0_20px_rgba(0,240,255,0.08)] transition-all duration-400 cursor-default group">
                  <UIcon name="simple-icons:express"
                    class="w-10 h-10 text-[#e3e2e2] group-hover:drop-shadow-[0_0_8px_rgba(255,255,255,0.3)] transition-all duration-300" />
                  <span
                    class="text-[10px] font-geist text-[#b9cacb] tracking-wider font-semibold group-hover:text-[#00f0ff] transition-colors duration-300">Express</span>
                </div>
                <div
                  class="tech-card flex flex-col items-center gap-3 p-4 rounded-xl border border-[#3b494b]/20 bg-[#1a1b1b]/30 backdrop-blur-md hover:border-[#00f0ff]/50 hover:scale-[1.05] hover:shadow-[0_0_20px_rgba(0,240,255,0.08)] transition-all duration-400 cursor-default group">
                  <UIcon name="logos:bun"
                    class="w-10 h-10 group-hover:drop-shadow-[0_0_8px_rgba(251,191,36,0.5)] transition-all duration-300" />
                  <span
                    class="text-[10px] font-geist text-[#b9cacb] tracking-wider font-semibold group-hover:text-[#00f0ff] transition-colors duration-300">Bun</span>
                </div>
                <div
                  class="tech-card flex flex-col items-center gap-3 p-4 rounded-xl border border-[#3b494b]/20 bg-[#1a1b1b]/30 backdrop-blur-md hover:border-[#00f0ff]/50 hover:scale-[1.05] hover:shadow-[0_0_20px_rgba(0,240,255,0.08)] transition-all duration-400 cursor-default group">
                  <img src="/elysia.svg" alt="Elysia" class="w-10 h-10" />
                  <span
                    class="text-[10px] font-geist text-[#b9cacb] tracking-wider font-semibold group-hover:text-[#00f0ff] transition-colors duration-300">Elysia</span>
                </div>
                <div
                  class="tech-card flex flex-col items-center gap-3 p-4 rounded-xl border border-[#3b494b]/20 bg-[#1a1b1b]/30 backdrop-blur-md hover:border-[#00f0ff]/50 hover:scale-[1.05] hover:shadow-[0_0_20px_rgba(0,240,255,0.08)] transition-all duration-400 cursor-default group">
                  <UIcon name="logos:go"
                    class="w-10 h-10 group-hover:drop-shadow-[0_0_8px_rgba(0,173,216,0.5)] transition-all duration-300" />
                  <span
                    class="text-[10px] font-geist text-[#b9cacb] tracking-wider font-semibold group-hover:text-[#00f0ff] transition-colors duration-300">Golang</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div v-show="progress >= 0.45 && progress <= 0.75"
            ...
            <div class="max-w-[620px] w-full flex flex-col"
              :class="connectedTabsCount > 0 ? 'items-start text-left' : 'items-center lg:items-start text-center lg:text-left'">
              <div
                class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-[#3b494b]/30 bg-[#1a1b1b]/40 w-fit mb-6"
                :class="connectedTabsCount > 0 ? 'mx-0' : 'mx-auto lg:mx-0'">
                <span class="w-1.5 h-1.5 rounded-full bg-[#ff0080]"></span>
                <span
                  class="font-geist text-[9px] text-[#b9cacb] uppercase tracking-[0.2em] font-semibold">Infrastructure</span>
              </div>
              <h2 class="font-hanken font-bold tracking-tight text-[#e3e2e2] mb-8"
                :class="connectedTabsCount > 0 ? 'text-5xl' : 'text-3xl md:text-5xl'">
              ...
              <div class="grid gap-3 pointer-events-auto w-full"
                ...
                <div
                  ...
                  <span
                    class="text-[10px] font-geist text-[#b9cacb] tracking-wider font-semibold group-hover:text-[#00f0ff] transition-colors duration-300">PostgreSQL</span>
                </div>
                <div
                  class="tech-card flex flex-col items-center gap-3 p-4 rounded-xl border border-[#3b494b]/20 bg-[#1a1b1b]/30 backdrop-blur-md hover:border-[#00f0ff]/50 hover:scale-[1.05] hover:shadow-[0_0_20px_rgba(0,240,255,0.08)] transition-all duration-400 cursor-default group">
                  <UIcon name="logos:mysql" class="w-10 h-10" />
                  <span
                    class="text-[10px] font-geist text-[#b9cacb] tracking-wider font-semibold group-hover:text-[#00f0ff] transition-colors duration-300">MySQL</span>
                </div>
                <div
                  class="tech-card flex flex-col items-center gap-3 p-4 rounded-xl border border-[#3b494b]/20 bg-[#1a1b1b]/30 backdrop-blur-md hover:border-[#00f0ff]/50 hover:scale-[1.05] hover:shadow-[0_0_20px_rgba(0,240,255,0.08)] transition-all duration-400 cursor-default group">
                  <UIcon name="logos:redis" class="w-10 h-10" />
                  <span
                    class="text-[10px] font-geist text-[#b9cacb] tracking-wider font-semibold group-hover:text-[#00f0ff] transition-colors duration-300">Redis</span>
                </div>
                <div
                  class="tech-card flex flex-col items-center gap-3 p-4 rounded-xl border border-[#3b494b]/20 bg-[#1a1b1b]/30 backdrop-blur-md hover:border-[#00f0ff]/50 hover:scale-[1.05] hover:shadow-[0_0_20px_rgba(0,240,255,0.08)] transition-all duration-400 cursor-default group">
                  <UIcon name="logos:aws-s3" class="w-10 h-10" />
                  <span
                    class="text-[10px] font-geist text-[#b9cacb] tracking-wider font-semibold group-hover:text-[#00f0ff] transition-colors duration-300">S3</span>
                </div>
                <div
                  class="tech-card flex flex-col items-center gap-3 p-4 rounded-xl border border-[#3b494b]/20 bg-[#1a1b1b]/30 backdrop-blur-md hover:border-[#00f0ff]/50 hover:scale-[1.05] hover:shadow-[0_0_20px_rgba(0,240,255,0.08)] transition-all duration-400 cursor-default group">
                  <UIcon name="logos:postman-icon" class="w-10 h-10" />
                  <span
                    class="text-[10px] font-geist text-[#b9cacb] tracking-wider font-semibold group-hover:text-[#00f0ff] transition-colors duration-300">Postman</span>
                </div>
                <div
                  class="tech-card flex flex-col items-center gap-3 p-4 rounded-xl border border-[#3b494b]/20 bg-[#1a1b1b]/30 backdrop-blur-md hover:border-[#00f0ff]/50 hover:scale-[1.05] hover:shadow-[0_0_20px_rgba(0,240,255,0.08)] transition-all duration-400 cursor-default group">
                  <UIcon name="logos:swagger" class="w-10 h-10" />
                  <span
                    class="text-[10px] font-geist text-[#b9cacb] tracking-wider font-semibold group-hover:text-[#00f0ff] transition-colors duration-300">Swagger</span>
                </div>
                <div
                  class="tech-card flex flex-col items-center gap-3 p-4 rounded-xl border border-[#3b494b]/20 bg-[#1a1b1b]/30 backdrop-blur-md hover:border-[#00f0ff]/50 hover:scale-[1.05] hover:shadow-[0_0_20px_rgba(0,240,255,0.08)] transition-all duration-400 cursor-default group">
                  <UIcon name="logos:supabase-icon" class="w-10 h-10" />
                  <span
                    class="text-[10px] font-geist text-[#b9cacb] tracking-wider font-semibold group-hover:text-[#00f0ff] transition-colors duration-300">Supabase</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div v-show="progress >= 0.8 && progress <= 1.1"
            ...
            <div class="max-w-[620px] flex flex-col gap-6 w-full"
              :class="connectedTabsCount > 0 ? 'text-left' : 'text-center lg:text-left'">
              <h2 class="font-hanken font-bold tracking-tight text-[#e3e2e2] leading-tight"
                :class="connectedTabsCount > 0 ? 'text-5xl' : 'text-4xl md:text-5xl'">
                {{ selectedLocale === 'id' ? 'Selamat Datang Di Web Portofolio Saya' : 'Welcome to My Web Portfolio' }}
              </h2>
              <p class="font-geist text-[#b9cacb] font-light tracking-wide max-w-xl"
                :class="connectedTabsCount > 0 ? 'text-lg mx-0' : 'text-base md:text-lg mx-auto lg:mx-0'">
      ...
      <section class="max-w-[1400px] mx-auto py-24 md:py-32" :class="connectedTabsCount > 0 ? 'px-16' : 'px-6 md:px-16'"
        id="career">
        <div class="section-reveal" :class="{ 'is-visible': revealSections.career }">
          ...
          <div class="mb-16 md:mb-20 flex flex-col md:flex-row md:items-end md:justify-between gap-6">
            <div>
              <div
                ...
                <span
                  class="font-geist text-[9px] text-[#b9cacb] uppercase tracking-[0.2em] font-semibold">Experience</span>
              </div>
              <h2 class="font-hanken font-bold text-[#e3e2e2] mb-3"
                :class="connectedTabsCount > 0 ? 'text-5xl' : 'text-3xl md:text-4xl lg:text-5xl'">
                {{ $t('career_arc_title') }}
              </h2>
              <p class="font-geist text-[#b9cacb] max-w-2xl font-light leading-relaxed"
                :class="connectedTabsCount > 0 ? 'text-lg' : 'text-base md:text-lg'">
                {{ $t('career_arc_subtitle') }}
              </p>
            </div>
            <a :href="cvDownloadUrl" @click.prevent="downloadCV"
              class="shrink-0 inline-flex items-center gap-2 text-[#00f0ff] hover:text-[#7df4ff] font-geist text-xs font-bold uppercase tracking-wider transition-colors duration-300">
              {{ $t('download_cv') }}
          ...
          <!-- Timeline -->
          <div class="relative border-l border-[#3b494b]/20 pb-8"
            :class="connectedTabsCount > 0 ? 'ml-8' : 'ml-4 md:ml-8'">
            <div v-for="(item, index) in experience" :key="index" class="timeline-item relative pb-14 group last:pb-0"
              :class="[connectedTabsCount > 0 ? 'pl-12' : 'pl-8 md:pl-12', { 'is-visible': revealSections.career }]"
              :style="{ '--delay': `${index * 100}ms` }">

              <!-- Timeline Node -->
              <div
                class="absolute -left-[5px] top-2 w-[9px] h-[9px] rounded-full bg-[#0a0b0b] border-2 border-[#3b494b]/50 group-hover:border-[#00f0ff] group-hover:bg-[#00f0ff] group-hover:shadow-[0_0_15px_rgba(0,240,255,0.6)] transition-all duration-500">
              </div>

              <!-- Card -->
              <div
                class="p-6 md:p-8 rounded-2xl border border-[#3b494b]/15 bg-[#1a1b1b]/20 backdrop-blur-sm hover:border-[#00f0ff]/30 hover:bg-[#1a1b1b]/40 transition-all duration-500 group/card hover:shadow-[0_0_30px_rgba(0,240,255,0.05)]">
                <div class="flex justify-between gap-3 mb-4"
                  :class="connectedTabsCount > 0 ? 'flex-row items-baseline' : 'flex-col md:flex-row md:items-baseline'">
                  <div class="flex flex-col">
                    <h3 :class="connectedTabsCount > 0 ? 'text-2xl' : 'text-xl md:text-2xl'"
                      class="font-hanken font-semibold text-[#e3e2e2] group-hover:text-[#00f0ff] transition-colors duration-300">
                      {{ item.title }}
                    </h3>
                  ...
                  <span
                    class="font-geist text-[10px] text-[#b9cacb] font-semibold uppercase tracking-widest bg-[#0a0b0b]/60 px-3 py-1.5 rounded-full border border-[#3b494b]/20 shrink-0">
                    {{ item.date }}
                  </span>
                </div>
                <p :class="connectedTabsCount > 0 ? 'text-base' : 'text-sm md:text-base'"
                  class="font-geist text-[#b9cacb]/70 leading-relaxed font-light">
                  {{ item.desc }}
                </p>
      ...
      <!-- ═══ Public APIs Section ═══ -->
      <section class="max-w-[1400px] mx-auto py-24 md:py-32 border-t border-[#3b494b]/10"
        :class="connectedTabsCount > 0 ? 'px-16' : 'px-6 md:px-16'" id="api-public">
        <div class="section-reveal" :class="{ 'is-visible': revealSections.apis }">
          <div class="mb-16 md:mb-20">
            <div
              ...
              <span
                class="font-geist text-[9px] text-[#b9cacb] uppercase tracking-[0.2em] font-semibold">Services</span>
            </div>
            <h2 class="font-hanken font-bold text-[#e3e2e2] mb-3"
              :class="connectedTabsCount > 0 ? 'text-5xl' : 'text-3xl md:text-4xl lg:text-5xl'">
              {{ $t('api_public_title') }}
          ...
          <div class="grid gap-6" :class="connectedTabsCount > 0 ? 'grid-cols-2' : 'grid-cols-1 md:grid-cols-2'">
            ...
            <a href="https://auth.syahendra.com/docs/" target="_blank" rel="noopener noreferrer"
              ...
              <div
                class="absolute -top-20 -right-20 w-[300px] h-[300px] bg-[#00f0ff]/3 blur-[100px] rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500">
              </div>
            ...
            <a href="https://provider.syahendra.com/" target="_blank" rel="noopener noreferrer"
              class="api-card group block relative overflow-hidden rounded-2xl border border-[#3b494b]/15 bg-gradient-to-br from-[#1a1b1b]/30 to-[#121414]/50 p-8 md:p-10 hover:border-[#00f0ff]/40 transition-all duration-500 hover:shadow-[0_0_40px_rgba(0,240,255,0.06)]">
              <div
                class="absolute inset-0 bg-gradient-to-br from-[#ff0080]/5 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500">
              </div>
              <div
                class="absolute -bottom-20 -left-20 w-[300px] h-[300px] bg-[#ff0080]/3 blur-[100px] rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500">
              </div>
                ...
                <div>
                  <div class="flex items-center justify-between gap-3 mb-8">
                    <span
                      ...
                      <span class="flex items-center gap-1.5">
                        <span
                          class="w-1.5 h-1.5 rounded-full bg-[#ff0080] animate-pulse shadow-[0_0_6px_rgba(255,0,128,0.4)]"></span>
                        Utility API
                      </span>
                    </span>
                    <span
                  ...
                  <div class="flex items-center gap-4 mb-5">
                    <div
                      class="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#ff0080]/10 to-[#ff0080]/5 flex items-center justify-center border border-[#ff0080]/10 group-hover:border-[#ff0080]/20 group-hover:bg-[#ff0080]/15 transition-all duration-300">
      ...
      <section class="max-w-[1400px] mx-auto py-24 md:py-32 border-t border-[#3b494b]/10"
        :class="connectedTabsCount > 0 ? 'px-16' : 'px-6 md:px-16'" id="articles">
        <div class="section-reveal" :class="{ 'is-visible': revealSections.articles }">
          <div class="mb-16 md:mb-20 flex justify-between gap-8"
            :class="connectedTabsCount > 0 ? 'flex-row items-end' : 'flex-col md:flex-row md:items-end'">
            <div>
              <div
                class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-[#3b494b]/30 bg-[#1a1b1b]/40 mb-6">
              ...
              <h2 :class="connectedTabsCount > 0 ? 'text-5xl' : 'text-3xl md:text-4xl lg:text-5xl'"
                class="font-hanken font-bold text-[#e3e2e2] mb-3">
                {{ $t('latest_writings_title') }}
              </h2>
              <p :class="connectedTabsCount > 0 ? 'text-lg' : 'text-base md:text-lg'"
                class="font-geist text-[#b9cacb] max-w-2xl font-light leading-relaxed">
          ...
          <div id="work" :class="connectedTabsCount > 0 ? 'grid-cols-2' : 'grid-cols-1 md:grid-cols-2'"
            class="grid gap-6">
            <NuxtLink v-for="(post, index) in paginatedPosts" :key="post._path" :to="post._path"
              ...
              <div
                class="absolute inset-0 bg-gradient-to-br from-[#00f0ff]/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500">
              </div>
              ...
              <div class="relative z-10 flex flex-col h-full justify-between gap-8">
                <div>
                  <div class="flex items-center justify-between gap-3 mb-6">
                    <div
                      class="flex items-center gap-2 px-2.5 py-1.5 bg-[#0a0b0b]/80 border border-[#3b494b]/20 rounded-lg font-geist text-[9px] text-[#b9cacb] uppercase tracking-[0.15em] font-semibold">
                      <span class="w-1.5 h-1.5 rounded-full bg-[#7df4ff]"></span>
                      <span class="line-clamp-1">Article</span>
                    </div>
                    <span class="text-[10px] font-geist text-[#b9cacb]/30 font-semibold shrink-0 font-mono">{{ post.date
                    }}</span>
                  </div>
                  <h4 :class="connectedTabsCount > 0 ? 'text-2xl' : 'text-xl md:text-2xl'"
                    class="font-hanken text-[#e3e2e2] group-hover:text-[#00f0ff] transition-colors duration-300 mb-4 font-bold leading-snug line-clamp-2">
                    {{ post.title }}
                  </h4>
                  <p class="font-geist text-xs text-[#b9cacb]/50 leading-relaxed font-light line-clamp-2">{{
                    post.description }}</p>
                </div>
                <div class="flex items-center justify-between pt-3 border-t border-[#3b494b]/10">
                  <span class="text-[10px] font-geist text-[#b9cacb]/40 font-medium">{{ post.readingTime || '3 min read'
                  }}</span>
                  <div
                    class="flex items-center text-[#b9cacb]/50 group-hover:text-[#e3e2e2] font-geist text-xs uppercase tracking-widest font-bold transition-all duration-300">
                    {{ $t('read_article') }}
          ...
          <!-- Pagination -->
          <div v-if="totalPages > 1" :class="connectedTabsCount > 0 ? 'flex-row' : 'flex-col sm:flex-row'"
            class="mt-16 flex items-center justify-between gap-8 border-t border-[#3b494b]/10 pt-12">
            <div class="flex items-center gap-4">
              ...
              <div class="h-1 w-20 bg-[#1a1b1b] rounded-full overflow-hidden">
                <div class="h-full bg-[#00f0ff] rounded-full transition-all duration-500"
    ...
    <footer class="w-full bg-[#0a0b0b] border-t border-[#3b494b]/10 relative" id="connect">
      <!-- Decorative top glow -->
      <div
        class="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[1px] bg-gradient-to-r from-transparent via-[#00f0ff]/30 to-transparent">
      </div>
      ...
      <div class="max-w-[1400px] mx-auto py-20 md:py-28" :class="connectedTabsCount > 0 ? 'px-16' : 'px-6 md:px-16'">
        <div class="section-reveal" :class="{ 'is-visible': revealSections.connect }">
          ...
          <!-- CTA Header -->
          <div :class="connectedTabsCount > 0 ? 'grid-cols-2' : 'grid-cols-1 md:grid-cols-2'"
            class="mb-20 pb-20 border-b border-[#3b494b]/10 grid gap-12 items-end">
            <div>
              <div
                class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-[#3b494b]/30 bg-[#1a1b1b]/40 mb-6">
                <span class="w-1.5 h-1.5 rounded-full bg-[#00f0ff] animate-pulse"></span>
                <span class="font-geist text-[9px] text-[#b9cacb] uppercase tracking-[0.2em] font-semibold">Get in
                  Touch</span>
              </div>
              <h3 :class="connectedTabsCount > 0 ? 'text-5xl' : 'text-4xl md:text-5xl'"
                class="font-hanken font-bold text-[#e3e2e2] mb-3 leading-tight">
                {{ $t('lets_build_something') }}
              </h3>
              <p :class="connectedTabsCount > 0 ? 'text-base' : 'text-sm md:text-base'"
                class="font-geist text-[#b9cacb] font-light leading-relaxed">
                {{ $t('available_for_opportunities') }}
              </p>
            </div>
            <div :class="connectedTabsCount > 0 ? 'items-end' : 'flex-col md:items-end'"
              class="flex flex-col gap-4 font-geist text-sm font-semibold">
              <a class="text-[#00f0ff] hover:text-[#7df4ff] transition-colors duration-300 inline-flex items-center gap-3 group"
                href="mailto:muhsyahendraa1722@gmail.com">
                <div
                  class="w-10 h-10 rounded-xl bg-[#00f0ff]/10 flex items-center justify-center group-hover:bg-[#00f0ff]/20 transition-colors duration-300">
                  <UIcon name="i-heroicons-envelope-20-solid" class="w-5 h-5" />
                </div>
                muhsyahendraa1722@gmail.com
              </a>
              <a class="text-[#b9cacb] hover:text-[#e3e2e2] transition-colors duration-300 inline-flex items-center gap-3 group"
                href="tel:+6289663604258">
                <div
                  class="w-10 h-10 rounded-xl bg-[#3b494b]/20 flex items-center justify-center group-hover:bg-[#3b494b]/40 transition-colors duration-300">
          ...
          <!-- Bottom Row -->
          <div :class="connectedTabsCount > 0 ? 'flex-row' : 'flex-col md:flex-row'"
            ...
            <nav class="flex items-center gap-5">
              <a href="https://github.com/hendras1722" target="_blank" rel="noopener noreferrer"
                class="w-10 h-10 rounded-xl border border-[#3b494b]/20 bg-[#1a1b1b]/20 flex items-center justify-center hover:border-[#00f0ff]/50 hover:bg-[#00f0ff]/5 hover:shadow-[0_0_10px_rgba(0,240,255,0.1)] transition-all duration-300 group"
                aria-label="GitHub">
                <svg class="w-4 h-4 text-[#b9cacb] group-hover:text-[#00f0ff] transition-colors duration-300"
              ...
              <a href="mailto:muhsyahendraa1722@gmail.com"
                ...
                <UIcon name="i-heroicons-envelope"
                  class="w-4 h-4 text-[#b9cacb] group-hover:text-[#00f0ff] transition-colors duration-300" />
              </a>
              <a :href="cvDownloadUrl" @click.prevent="downloadCV"
                class="w-10 h-10 rounded-xl border border-[#3b494b]/20 bg-[#1a1b1b]/20 flex items-center justify-center hover:border-[#00f0ff]/50 hover:bg-[#00f0ff]/5 hover:shadow-[0_0_10px_rgba(0,240,255,0.1)] transition-all duration-300 group"
                aria-label="CV">
    ...
    <div v-if="isDownloadingCV"
      ...
      <div
        ...
        <svg class="animate-spin h-10 w-10 text-[#00f0ff] mb-4" xmlns="http://www.w3.org/2000/svg" fill="none"
          viewBox="0 0 24 24">
