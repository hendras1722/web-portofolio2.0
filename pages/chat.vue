<template>
  <div class="lg:px-[160px] px-[20px]">
    ...
    <div class="flex justify-center shadow-lg p-10 rounded-lg mt-16">
      <div class="w-full">
        <div id="container_typing" class="border border-gray-100 p-6 mb-5 rounded-lg h-[440px] overflow-auto">
          <div v-for="(item, index) in modelValue" :key="index">
            ...
            <div class="flex items-start mb-5" v-if="item.id !== id">
              <div class="grid place-items-start">
                ...
                <div class="min-w-min my-1 whitespace-pre-wrap">
                  <div class="shadow-md p-2 rounded-lg max-w-2xl min-w-min whitespace-pre-wrap">
                    <p>
                      {{ item.chat }}
                    </p>
                  </div>
            ...
            <div v-if="item.id === id">
              <div class="w-full flex justify-end">
                <div class="text-sm text-gray-400 text-end font-bold truncate w-52">
                  {{ item.info.full_name }}
                </div>
              </div>
        ...
        <div class="flex items-center" v-if="user">
          <div class="p-2 w-full border border-green-300 bg-green-50 min-h-10 rounded">
            <div id="typing" contenteditable @focus="handleFocus" @blur="handleBlur" class="outline-none"></div>
          </div>
          <div class="w-min">
            <UButton class="ml-5 rounded-full" :ui="{ base: '!bg-green-500 hover:!bg-green-400' }"
              icon="i-ic-baseline-send" @click="handleSend">
            </UButton>
          </div>
        </div>
        ...
        <div class="text-center mt-5" v-if="user">
          <UButton @click="handleOut" variant="ghost" color="red">Sign Out</UButton>
        </div>
      </div>
    </div>
  </div>
